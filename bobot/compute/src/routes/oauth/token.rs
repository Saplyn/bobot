...
pub async fn handler(
    ...
    let span = span!(Level::DEBUG, "oauth-token");

    // Reconstruct token URL
    let mut url = match Url::from_str(super::QQ_OAUTH_TOKEN_URL) {
        Ok(url) => url,
        Err(error) => {
            span.in_scope(|| error!(message = "Failed to parse QQ's OAuth token URL", %error));
    ...
    drop(url_queries);
    span.in_scope(|| debug!(message = "Reconstructed token URL", %url)); // FIXME:

    // Call QQ's OAuth token URL
    let resp = match app_state.reqwest.get(url).send().await {
        Ok(resp) => resp,
        Err(error) => {
            span.in_scope(|| error!(message = "Failed to call QQ's OAuth token URL", %error));
            return http::StatusCode::INTERNAL_SERVER_ERROR.into_response();
        }
    };
    let token = match resp.json::<serde_json::Value>().await {
        Ok(token) => token,
        Err(error) => {
            span.in_scope(|| error!(message = "Failed to parse QQ's OAuth token response", %error));
            return http::StatusCode::INTERNAL_SERVER_ERROR.into_response();
        }
    };

    span.in_scope(|| error!(message = "Got QQ's OAuth token response ", response = %token));
